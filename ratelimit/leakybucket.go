...
type UberLeakyBucket struct {
	rate    int
	limiter rl.Limiter
}

func NewLeakyBucket(rate int) *UberLeakyBucket {
	return &UberLeakyBucket{
		rate:    rate,
		limiter: rl.New(rate, rl.WithoutSlack),
	}
}

func (t *UberLeakyBucket) Take() time.Time {
