	...
	go func() {
		defer func() {
			if err := recover(); err != nil {
				c.chOpen = false
				fmt.Printf("send email queue err: %+v, retry after 10 second", err.(error))
		...
		for {
			...
			case m, ok := <-c.ch:
				if !ok {
					fmt.Printf("mail queue is close")
					c.chOpen = false
					return
				}
				if !open {
				...
				if err := mail.Send(s, m); err != nil {
					fmt.Printf("email send failed, %v", err)
				} else {
					fmt.Printf("email has send")
				}
			case <-time.After(time.Duration(c.Config.Keepalive) * time.Second):
				if open {
					if err := s.Close(); err != nil {
						fmt.Printf("can not close smtp conn, %v", err)
					}
				}
				open = false
			}
