...
var (
	cacheSysPermissionByIDPrefix = "DBCache:user:SysPermissionByID"
)
...
type (
	ISysPermissionRepo interface {
		...
		FindOneCacheByID(ctx context.Context, ID string) (*user_model.SysPermission, error)
		// FindOneByID 根据ID查询一条数据
		FindOneByID(ctx context.Context, ID string) (*user_model.SysPermission, error)
		// FindMultiCacheByIDS 根据IDS查询多条数据并设置缓存
		FindMultiCacheByIDS(ctx context.Context, IDS []string) ([]*user_model.SysPermission, error)
		// FindMultiByIDS 根据IDS查询多条数据
		FindMultiByIDS(ctx context.Context, IDS []string) ([]*user_model.SysPermission, error)
		...
		DeleteUniqueIndexCache(ctx context.Context, data []*user_model.SysPermission) error
	}
...
// UpdateOne 更新一条数据
func (r *SysPermissionRepo) UpdateOne(ctx context.Context, data *user_model.SysPermission) error {
	dao := user_dao.Use(r.db).SysPermission
	_, err := dao.WithContext(ctx).Where(dao.ID.Eq(data.ID)).Updates(data)
	if err != nil {
		return err
	}
	err = r.DeleteUniqueIndexCache(ctx, []*user_model.SysPermission{data})
	if err != nil {
		return err
	}
	return nil
...
func (r *SysPermissionRepo) DeleteOneCacheByID(ctx context.Context, ID string) error {
	...
	err = r.DeleteUniqueIndexCache(ctx, []*user_model.SysPermission{first})
	if err != nil {
		return err
	}
	return nil
...
// DeleteUniqueIndexCache 删除唯一索引存在的缓存
func (r *SysPermissionRepo) DeleteUniqueIndexCache(ctx context.Context, data []*user_model.SysPermission) error {
	keys := make([]string, 0)
	for _, v := range data {
		keys = append(keys, r.cache.Key(ctx, cacheSysPermissionByIDPrefix, v.ID))
...
// FindOneCacheByID 根据ID查询一条数据并设置缓存
func (r *SysPermissionRepo) FindOneCacheByID(ctx context.Context, ID string) (*user_model.SysPermission, error) {
	resp := new(user_model.SysPermission)
	key := r.cache.Key(ctx, cacheSysPermissionByIDPrefix, ID)
...
// FindOneByID 根据ID查询一条数据
func (r *SysPermissionRepo) FindOneByID(ctx context.Context, ID string) (*user_model.SysPermission, error) {
	dao := user_dao.Use(r.db).SysPermission
	result, err := dao.WithContext(ctx).Where(dao.ID.Eq(ID)).First()
	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, err
...
// FindMultiCacheByIDS 根据IDS查询多条数据并设置缓存
func (r *SysPermissionRepo) FindMultiCacheByIDS(ctx context.Context, IDS []string) ([]*user_model.SysPermission, error) {
	resp := make([]*user_model.SysPermission, 0)
	keys := make([]string, 0)
	keyToParam := make(map[string]string)
	for _, v := range IDS {
		key := r.cache.Key(ctx, cacheSysPermissionByIDPrefix, v)
	...
	cacheValue, err := r.cache.FetchBatch(ctx, keys, func(miss []string) (map[string]string, error) {
		...
		for _, v := range result {
			...
			value[r.cache.Key(ctx, cacheSysPermissionByIDPrefix, v.ID)] = string(marshal)
		}
		return value, nil
	})
	if err != nil {
	...
	for _, v := range cacheValue {
		tmp := new(user_model.SysPermission)
...
// FindMultiByIDS 根据IDS查询多条数据
func (r *SysPermissionRepo) FindMultiByIDS(ctx context.Context, IDS []string) ([]*user_model.SysPermission, error) {
	dao := user_dao.Use(r.db).SysPermission
