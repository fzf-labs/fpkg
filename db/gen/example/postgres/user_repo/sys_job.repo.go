...
var (
	cacheSysJobByIDPrefix = "DBCache:user:SysJobByID"
)
...
type (
	ISysJobRepo interface {
		...
		FindOneCacheByID(ctx context.Context, ID string) (*user_model.SysJob, error)
		// FindOneByID 根据ID查询一条数据
		FindOneByID(ctx context.Context, ID string) (*user_model.SysJob, error)
		// FindMultiCacheByIDS 根据IDS查询多条数据并设置缓存
		FindMultiCacheByIDS(ctx context.Context, IDS []string) ([]*user_model.SysJob, error)
		// FindMultiByIDS 根据IDS查询多条数据
		FindMultiByIDS(ctx context.Context, IDS []string) ([]*user_model.SysJob, error)
		...
		DeleteUniqueIndexCache(ctx context.Context, data []*user_model.SysJob) error
	}
...
// UpdateOne 更新一条数据
func (r *SysJobRepo) UpdateOne(ctx context.Context, data *user_model.SysJob) error {
	dao := user_dao.Use(r.db).SysJob
	_, err := dao.WithContext(ctx).Where(dao.ID.Eq(data.ID)).Updates(data)
	if err != nil {
		return err
	}
	err = r.DeleteUniqueIndexCache(ctx, []*user_model.SysJob{data})
	if err != nil {
		return err
	}
	return nil
...
func (r *SysJobRepo) DeleteOneCacheByID(ctx context.Context, ID string) error {
	...
	err = r.DeleteUniqueIndexCache(ctx, []*user_model.SysJob{first})
	if err != nil {
		return err
	}
	return nil
...
// DeleteUniqueIndexCache 删除唯一索引存在的缓存
func (r *SysJobRepo) DeleteUniqueIndexCache(ctx context.Context, data []*user_model.SysJob) error {
	keys := make([]string, 0)
	for _, v := range data {
		keys = append(keys, r.cache.Key(ctx, cacheSysJobByIDPrefix, v.ID))
...
// FindOneCacheByID 根据ID查询一条数据并设置缓存
func (r *SysJobRepo) FindOneCacheByID(ctx context.Context, ID string) (*user_model.SysJob, error) {
	resp := new(user_model.SysJob)
	key := r.cache.Key(ctx, cacheSysJobByIDPrefix, ID)
...
// FindOneByID 根据ID查询一条数据
func (r *SysJobRepo) FindOneByID(ctx context.Context, ID string) (*user_model.SysJob, error) {
	dao := user_dao.Use(r.db).SysJob
	result, err := dao.WithContext(ctx).Where(dao.ID.Eq(ID)).First()
	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, err
...
// FindMultiCacheByIDS 根据IDS查询多条数据并设置缓存
func (r *SysJobRepo) FindMultiCacheByIDS(ctx context.Context, IDS []string) ([]*user_model.SysJob, error) {
	resp := make([]*user_model.SysJob, 0)
	keys := make([]string, 0)
	keyToParam := make(map[string]string)
	for _, v := range IDS {
		key := r.cache.Key(ctx, cacheSysJobByIDPrefix, v)
	...
	cacheValue, err := r.cache.FetchBatch(ctx, keys, func(miss []string) (map[string]string, error) {
		...
		for _, v := range result {
			...
			value[r.cache.Key(ctx, cacheSysJobByIDPrefix, v.ID)] = string(marshal)
		}
		return value, nil
	})
	if err != nil {
...
// FindMultiByIDS 根据IDS查询多条数据
func (r *SysJobRepo) FindMultiByIDS(ctx context.Context, IDS []string) ([]*user_model.SysJob, error) {
	dao := user_dao.Use(r.db).SysJob
