...
func newSysRole(db *gorm.DB, opts ...gen.DOOption) sysRole {
	...
	_sysRole.sysRoleDo.UseModel(&user_model.SysRole{})
...
func (s sysRoleDo) Create(values ...*user_model.SysRole) error {
	if len(values) == 0 {
		return nil
	}
...
func (s sysRoleDo) CreateInBatches(values []*user_model.SysRole, batchSize int) error {
	return s.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (s sysRoleDo) Save(values ...*user_model.SysRole) error {
	if len(values) == 0 {
		return nil
	}
...
func (s sysRoleDo) First() (*user_model.SysRole, error) {
	if result, err := s.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*user_model.SysRole), nil
	}
}

func (s sysRoleDo) Take() (*user_model.SysRole, error) {
...
func (s sysRoleDo) Last() (*user_model.SysRole, error) {
...
func (s sysRoleDo) Find() ([]*user_model.SysRole, error) {
	result, err := s.DO.Find()
	return result.([]*user_model.SysRole), err
}

func (s sysRoleDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*user_model.SysRole, err error) {
	buf := make([]*user_model.SysRole, 0, batchSize)
...
func (s sysRoleDo) FindInBatches(result *[]*user_model.SysRole, batchSize int, fc func(tx gen.Dao, batch int) error) error {
...
func (s sysRoleDo) FirstOrInit() (*user_model.SysRole, error) {
...
func (s sysRoleDo) FirstOrCreate() (*user_model.SysRole, error) {
...
func (s sysRoleDo) FindByPage(offset int, limit int) (result []*user_model.SysRole, count int64, err error) {
...
func (s sysRoleDo) Delete(models ...*user_model.SysRole) (result gen.ResultInfo, err error) {
