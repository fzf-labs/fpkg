...
type (
	ISysLogRepo interface {
		...
		FindMultiByPaginator(ctx context.Context, params *orm.PaginatorParams) ([]*gorm_gen_model.SysLog, int64, error)
...
// FindMultiByPaginator 查询分页数据(通用)
func (s *SysLogRepo) FindMultiByPaginator(ctx context.Context, params *orm.PaginatorParams) ([]*gorm_gen_model.SysLog, int64, error) {
	result := make([]*gorm_gen_model.SysLog, 0)
	var total int64
	queryStr, args, err := params.ConvertToGormConditions()
	if err != nil {
		return nil, 0, err
	}
	err = s.db.WithContext(ctx).Model(&gorm_gen_model.SysLog{}).Select([]string{"id"}).Where(queryStr, args...).Count(&total).Error
	if err != nil {
		return nil, 0, err
	}
	if total == 0 {
