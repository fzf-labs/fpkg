...
type (
	...
	Cache struct {
		...
		lruCache       lru
		barrier        singleflight.SingleFlight
...
func (c *Cache) Del(key string) {
	...
	c.lock.Unlock()
	_ = c.timingWheel.RemoveTimer(key)
}
...
func (c *Cache) SetWithExpire(key string, value any, expire time.Duration) {
	...
	if ok {
		_ = c.timingWheel.MoveTimer(key, expiry)
	} else {
		_ = c.timingWheel.SetTimer(key, value, expiry)
	}
}
...
func (c *Cache) TakeWithExpire(key string, expire time.Duration, fetch func() (any, error)) (any, error) {
	if val, ok := c.doGet(key); ok {
		c.stats.IncrementHit()
		return val, nil
	}
...
func (l *keyLru) removeElement(e *list.Element) {
	l.evicts.Remove(e)
	key := e.Value.(string)
